...
public function getAcademicData($matricula)
	...
	public function procesarDatosMaterias($matricula){
		...
		$allProffNames = $this->getOnlyProffNames($matricula);

		foreach ($aux as $key => $value) {
			$nombres[]=$this->getMateriasNombres($key);
		}

		for($i = 0; $i <= (count($allProffNames) - 1); $i++)
		{
			$nombresProff[$i]=$allProffNames[$i];
		}
		...
		if(count($nombres)>0 && is_array($nombres)){
			foreach ($nombres as $indice=>$arrDatos){
				if(array_key_exists($arrDatos['clave_materia'],$aux))
				{
					...
					$aux[$arrDatos['clave_materia']]['nombre_profesor']=$nombresProff[$indice]['nombre_profesor'];
					$aux[$arrDatos['clave_materia']]['apellido_profesor']=$nombresProff[$indice]['apellido_profesor'];
					...
					if(strcmp($fila['PLACVE'],$clavePeriodo)==0){
 						if(strcmp($fila['MATCVE'],$materia)==0){
							$aux[$fila['MATCVE']][] =array(
								//'lunes'			=>substr(trim($fila['LUNHRA']),0,4),
								//'lunesOne'			=>substr(trim($fila['LUNHRA']),4,8),
								'lunes'			=>$fila['LUNHRA'],	
